...
class Melspec:
	...
	def process_melspectrogram(self, signal, fs, report = False):
		...
		starts  = starts[starts + self.n_fft < len(signal)]
	...
	def calculate_params(self, len_signal, fs, report = False):
		...
		starts  = starts[starts + self.n_fft < self.input_length]
		len_nfft_nmels = (self.n_fft//2)//self.n_mels
	...
	def plot(self,title = None, grayscale = False):
		...
		'''
		assert not self.spec is None, "You must execute the function process_melspectrogram() before graphing."
